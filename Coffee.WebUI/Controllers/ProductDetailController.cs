...
namespace Coffee.WebUI.Controllers
{
    public class ProductDetailController : Controller
    {
        ...
        public ProductDetailController(IRepository<Order> orderRepository, 
        ...
        public async Task<IActionResult> Index(int id)
        {
            ...
            {
                //var _rating = _review.Where(x => x.ProductId == id).Sum(x => x.Rating) / _review.Where(x => x.ProductId == id).Count();
                decimal _rating = Math.Round((decimal)(_review.Where(x => x.ProductId == id).Sum(x => x.Rating) / (decimal)_review.Where(x => x.ProductId == id).Count()), 2);

                //ViewBag.Rating = Math.Round(_rating, 0);
                ViewBag.Rating = _rating;
        ...
        public async Task<IActionResult> Like(int productId)
        {
            try
            {
                var _user = await _userRepository.GetAllAsync();
                var _userId = _user.First(x => x.Email == User.FindFirst(ClaimTypes.Email)?.Value).Id;
                var _productLike = await _productLikeRepository.GetByIdAsync(productId);
                if (_productLike == null)
                {
                    ...
                    await _productLikeRepository.InsertAsync(_productLike);
                }
                else
                {
                    _productLike.IsActive = !_productLike.IsActive;
                    await _productLikeRepository.UpdateAsync(_productLike);
                }
                return Json(new
                {
                    success = true,
